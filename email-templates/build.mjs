/* ─────────────────────────────────────────────────────────────────────
   Build the eight campaign emails as standalone HTML.

     node email-templates/build.mjs

   Source of truth is src/app/components/ds/campaign/issues.ts — the same
   file the design-system previews render from, transpiled here with the
   esbuild already in node_modules. Editing the copy in one place changes
   both the preview and the shipped HTML.

   Output targets GoHighLevel's HTML editor: one self-contained document
   per issue, tables only, inline styles, 600px, no external CSS.
───────────────────────────────────────────────────────────────────── */
...
/* ── Palette · light Inkwell. Mirrors EMAIL.light in email/kit.tsx. ── */
const C = {
  outer:      "#E4EDF2",
  body:       "#FFFFFF",
  surface:    "#F2F7FA",
...
const LOGO = "https://raw.githubusercontent.com/teamos-ai/capital-unique-design/main/src/imports/Logo_Square_Silver.png";

/* GoHighLevel merge fields. Confirm these against the field picker in
   your own sub-account before the first send — GHL can also append its
   own unsubscribe block, in which case delete the line below. */
const MERGE = {
...
function document_(issue, total) {
  return `<!DOCTYPE html>
...
const tmp = resolve(HERE, ".issues.tmp.mjs");
await build({
...
const { ISSUES } = await import(pathToFileURL(tmp).href);
rmSync(tmp);
...
for (const issue of ISSUES) {
  const name = `week-${String(issue.week).padStart(2, "0")}-${slug(issue.subject)}.html`;
  writeFileSync(resolve(OUT, name), document_(issue, ISSUES.length));
...
<style>
 body{margin:0;background:${C.outer};font-family:${SANS};color:${C.text};}
 h1{font-family:${SERIF};font-weight:600;font-size:28px;margin:0 0 6px;}
 .wrap{max-width:1000px;margin:0 auto;padding:40px 20px;}
 p.lede{color:${C.textSoft};margin:0 0 28px;line-height:1.6;}
 ol{padding-left:20px;margin:0;}
 li{margin-bottom:14px;line-height:1.5;}
 a{color:${C.accent};}
 code{background:${C.surface};padding:2px 6px;border-radius:3px;font-size:12px;}
 .sub{color:${C.textQuiet};font-size:13px;}
